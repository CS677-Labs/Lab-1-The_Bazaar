...
function finish {
  echo "Cleanup"
  rm -rf classfiles
  kill $server_id $client_id || echo "No processes to delete."
}
trap finish EXIT 1
...
#    role1 = "Server"
#    role2 = "Buyer"
#echo "Running Node 1 as a $role1 of Fishes."
#java -classpath classfiles -Djava.rmi.server.codebase=file:files/ $role1 1 src/config.properties Boar &
#
#echo "Running Node 2 as a $role2 of Fishes."
#java -classpath classfiles -Djava.rmi.server.codebase=file:files/ $role2 2 src/config.properties Fish &
#
#sleep 20
#if (grep -Fxq "Bought product Fish" src/client.log) && (grep -Fxq "Restocking" src/server.log)
#then
#    echo "Test case 2 Failed."
#    exit 1
#else
